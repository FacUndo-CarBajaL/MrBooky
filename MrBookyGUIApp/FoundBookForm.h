	...
	public:
		void ShowDetails(Book^ book) {
			...
			for each (String^ review in book->Reviews) {
				...
				dvgReviews->Rows[rowIndex]->Cells[0]->Value = review->ReviewerName;
				dvgReviews->Rows[rowIndex]->Cells[1]->Value = review->Rating;
				dvgReviews->Rows[rowIndex]->Cells[2]->Value = review->Comment;
			}
